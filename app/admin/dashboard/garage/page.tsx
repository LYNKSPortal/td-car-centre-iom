...
export default function GarageWorkPage() {
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Garage Work</h1>
          <p className="text-zinc-400">
            Manage general servicing and repair jobs
          </p>
        </div>
        <Button variant="primary" asChild>
          <Link href="/admin/dashboard/garage/new">
      ...
      <Suspense fallback={<GarageWorkListSkeleton />}>
        <GarageWorkList />
      </Suspense>
    </div>
  );
      ...
      <div className="bg-zinc-950 border border-white/10 rounded-lg p-12 text-center">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-zinc-400 mb-6">
            Get started by adding your first servicing or repair job.
          </p>
          <Button variant="primary" asChild>
            <Link href="/admin/dashboard/garage/new">
              <Plus className="w-4 h-4 mr-2" />
          ...
          <tbody className="divide-y divide-white/10">
            {garageJobs.map((job) => (
              <tr key={job.id} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-6 py-4">
                  <Link
                    href={`/admin/dashboard/garage/${job.id}`}
...
function GarageWorkListSkeleton() {
  return (
