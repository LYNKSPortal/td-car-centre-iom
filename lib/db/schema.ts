...
export const jobCards = pgTable('job_cards', {
  ...
  workRequested: text('work_requested'),
  healthChecks: text('health_checks'),
  partsRequired: text('parts_required'),
  workCarriedOut: text('work_carried_out'),
  additionalRepairs: text('additional_repairs'),
  comments: text('comments'),
  valeted: boolean('valeted').default(false),
  roadTested: boolean('road_tested').default(false),
  qualityCheck: boolean('quality_check').default(false),
  customerSignature: varchar('customer_signature', { length: 255 }),
  customerApprovalDate: varchar('customer_approval_date', { length: 50 }),
  completedBy: varchar('completed_by', { length: 255 }),
  dateCompleted: varchar('date_completed', { length: 50 }),
  finalInvoiceAmount: varchar('final_invoice_amount', { length: 50 }),
